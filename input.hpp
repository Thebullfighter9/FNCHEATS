...
namespace MouseController
{
	...
	static BOOLEAN WINAPI Init()
	{
		...
		LPVOID NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(skCrypt(L"win32u")), skCrypt("NtUserSendInput"));
		if (!NtUserSendInput_Addr)
		{
			NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(skCrypt(L"user32")), skCrypt("NtUserSendInput"));
			if (!NtUserSendInput_Addr)
			{
				// Windows 7 or lower detected
				NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(skCrypt(L"user32")), skCrypt("SendInput"));
				if (!NtUserSendInput_Addr)
					return FALSE;
			}
		}
		...
		// windows 8.1 / windows 10
		LPVOID NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(skCrypt(L"win32u")), skCrypt("NtUserGetAsyncKeyState"));
		if (!NtUserGetAsyncKeyState_Addr)
		{
			NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(skCrypt(L"user32")), skCrypt("NtUserGetAsyncKeyState"));
			if (!NtUserGetAsyncKeyState_Addr)
			{
				// Windows 7 or lower detected
				NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(skCrypt(L"user32")), skCrypt("GetAsyncKeyState"));
				if (!NtUserGetAsyncKeyState_Addr)
					return FALSE;
			}
		}

...
Camera GetCamera(__int64 a1)
{
	Camera LocalCamera;
	__int64 v1;
	v1 = Driver::read<__int64>(processID, Actors::Localplayer + 0xd0);
	const __int64 v9 = Driver::read<__int64>(processID, v1 + 0x8);
	LocalCamera.FieldOfView = 80.f / (Driver::read<double>(processID, v9 + 0x7F0) / 1.19f);
	LocalCamera.Rotation.x = Driver::read<double>(processID, v9 + 0x9C0);
	LocalCamera.Rotation.y = Driver::read<double>(processID, a1 + 0x148);
	uint64_t PointerLoc = Driver::read<uint64_t>(processID, Actors::Uworld + 0x110);
	LocalCamera.Location = Driver::read<Vector3>(processID, PointerLoc);
	return LocalCamera;
}
